...
# Call bert pretrained summarizer for 250 articles
bert_model = Summarizer('bert-base-uncased')
bert_summaries = [''.join(bert_model(x, num_sentences=3)) for x in df["articles"].iloc[:250]]

# get average Rouge scores
rouge = Rouge()
bert_scores = rouge.get_scores(bert_summaries, list(df["summary"].iloc[:250]), avg=True)
...
bleu_scores = [sentence_bleu([word_tokenize(ref)], word_tokenize(gen)) for ref, gen in zip(df["summary"].iloc[:250], bert_summaries[:250])]
average_bleu_score = sum(bleu_scores) / len(bleu_scores)
print("Average BLEU score:", average_bleu_score)

# Calculate METEOR scores for each pair of summaries and print out average
meteor_scores = [ms([word_tokenize(ref)], word_tokenize(gen)) for ref, gen in zip(list(df["summary"].iloc[:250]), bert_summaries)]
average_meteor_score = sum(meteor_scores) / len(meteor_scores)
print("Average METEOR Score:", average_meteor_score)

# Use a pre-trained BERT model ('bert-base-uncased')
P, R, F1 = score(bert_summaries, list(df["summary"].iloc[:250]), lang='en', model_type='bert-base-uncased')
